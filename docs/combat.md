...
## Real-time multiplayer skill-combat prototype

The first implementation keeps one authoritative simulation at native speed. It does not attempt player-local bullet time. Every mod-triggered Skill Strike is accepted only while `CGameSpeed` current/requested modes are both `0` and pause is clear. Player 1 continues through Sudeki's `ac_QuickSkill0..5` route; Player 2 `F1..F4` selects from its assigned character's own ordered SkillData and enters the same native availability, validation, and `CSkill::Use` functions. The design therefore retains self/radius/lock-on behavior, native SP cost, task protection, animation events, projectile/damage timing, and cleanup.

The exact build still has one observed global Skill Targeting context. The prototype measures ownership and held duration, but does not virtualize it prematurely. While the global active byte is set, another activation is rejected and logged with `native_global_targeting_busy`. After confirmation clears that byte, another player may start even if the first character's `CSkill+0x6C` remains active. This separates the unsafe selection overlap from the intended execution overlap.

Plasmatica is the first caster-only cinematic. Its confirmed script thread brackets calls to `CCameraManager::SetRenderCamera`; the hook resolves the requested camera and assigns its render state only to the caster's cached viewport. It does not write the manager's global render-camera pointer. The other player should continue moving and attacking with a stable gameplay camera while still seeing Elco's model animation in the shared world. Normal camera restore and native task cleanup both remove the local override. Live testing must still confirm that the cinematic camera object continues animating correctly while it is not globally selected.

The first live acceptance run must establish: world time stays normal throughout held targeting and execution; Buki movement/attack continues; only Elco's viewport enters the cinematic; a conflicting input during targeting rejects cleanly; an input after confirmation can overlap execution; cancellation/interruption/completion restore cameras, controls, targeting, and protection; and held targeting duration reveals whether native invulnerability is exploitable. No protection timing or balance value changes before that measurement.

### First integrated live observation — input and presentation remain global

The first run was intentionally treated as incomplete rather than accepted. The user confirmed that native mouse movement affected both character cameras, making independent aim and camera judgment impractical. This is expected from the current prototype: Player 2 receives only the injected keyboard movement/combat path, while both viewport render states still inherit Player 1's native global camera input. A physical second controller must be enumerated by Linux and Wine and routed to Player 2 before the independent-control acceptance test is meaningful.

Elco's ranged targeting presentation also remains process-global. From Buki's viewport, Elco was represented only by the first-person arm and gun rather than by his complete third-person world model. Caster-only camera routing therefore does not yet isolate Sudeki's first-person actor-visibility/model-mode switch. The eventual fix must make that visibility choice viewport-owned: Elco's own targeting view may retain the authored arms/gun presentation, while every other viewport renders his complete in-world actor and animation.

The runtime log independently rejected the real-time requirement. Immediately after Plasmatica activation, `CGameSpeed` changed from current/requested mode `0/0` to `2/2`; the context guard correctly rejected additional mod skill inputs during that interval, but it did not prevent the native cast from changing the shared mode. The mode returned to `0/0` when the skill completed. The next build must identify and suppress or locally replace this Skill Strike mode transition before claiming that enemies, AI, projectiles, and Player 2 remain at `1.0x` throughout targeting and execution.
