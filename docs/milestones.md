...
| Milestone 2: Plasmatica independent animation speed | Complete | Two 2.0x casts changed only Elco's validated `CNewMissileAimingGameModelAnimation` multiplier from `1.0` to `2.0` and restored it to `1.0`; event waits fell `201→101`, `249→125`, and `233→117` while normal world simulation remained enabled. The eye-view camera presentation was shortened/skipped and needs separate tuning. |
