...
| Plasmatica activation path | In progress | Runtime-confirmed as `PC_Elco1__Skill|P`; two animation events gate `FireMissileScripted(10)`; no known damage method fires on any script thread, and static analysis follows the native missile collision path; exact damage resolution and independent animation-speed control remain |
