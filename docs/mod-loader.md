...
`EnableExternalInputBridgePrototype` defaults to `false` and requires control separation plus Player 2 movement. It binds a nonblocking UDP receiver to `127.0.0.1:InputBridgePort` (default `26760`) and accepts only the exact version-1, 32-byte SudekiMP packet from loopback. `InputBridgeTimeoutMs` defaults to `250`; missing or stale packets immediately produce neutral axes/buttons rather than retaining the last action. `InputBridgeDeadzone` defaults to `0.20` and applies a radial rescale before the existing native camera transform and Player 2 arbiter movement call. The left and right sticks retain their movement and per-player camera roles outside the action router. Rising button edges use an explicit SudekiMP contextual policy: A resolves an exact interaction intent when a complete actor/target/source-generation tuple is already known, otherwise it submits native Weak; X submits native Strong for Tal/Buki, but toggles only the owning Player 2 viewport between its preserved third-person orbit and camera-only first-person view for Ailish/Elco. That ranged X behavior is an intentional SudekiMP exception to the shipped generic `X = Strong` label: exact-build analysis shows the ranged arbiter never reads its Strong argument, while native first-person hold/toggle actions remain separate. The viewport toggle never calls global `SetFirstPersonCameraMode`, changes the global controller/camera mode, or attaches the crash-prone first-person model wrapper. It therefore does not yet claim native arms/weapon presentation or Ailish magical-sight world effects. Y reports a per-seat Quick Menu intent but does not claim success because no per-seat native menu consumer is connected; B resolves modal Cancel or submits native Sweep in combat; and the D-pad reports per-seat Quickshot intents that are likewise not connected yet. Triggers still cross the protocol without a gameplay consumer. Edge state is maintained independently for four seats, and a disconnected/reconnected device must report neutral before another action edge is accepted.

`EnableNativeXInputPlayerTwoPrototype` defaults to `false`. It is the
Windows-only alternative to the loopback bridge, and the two sources are
mutually exclusive. `XInputPlayerTwoSlot` accepts only `0..3`; the first beta
profile uses the hardware-verified slot `0`. The DLL reads that controller
directly into the same normalized Player 2 state, then exact-build-gates the
game’s `XInputGetState` import and reports the reserved slot as disconnected to
the native game. This prevents one controller from also driving Player 1.
Every other XInput slot continues to use the game’s untouched native import.
If the source cannot open, the IAT does not match, or the controller later
disconnects, Player 2 becomes neutral rather than retaining input.
...
`EnableNativeSecondPlayerCameraCollisionPrototype` defaults to `false` and requires the Player 2 camera plus dual-frame stack. In ordinary Exploration it targets the named `SudekiMP_P2` camera at Player 2's live GEL entity and asks Sudeki's authored Exploration state to produce that viewport's matrix and obstruction response without taking global render ownership. Player 1 camera-input broadcasts are filtered only for this named camera. The native target-wrapper bridge completed a live Tal=P1/Ailish=P2 run through `target_verified`, `state_verified`, and `ready`; the normal `--party-lifecycle-trace` and `--door-transition-trace` profiles now enable it. Native readiness intentionally disables the manual Player 2 orbit because right-stick events are not yet translated into the native camera event stream; manual right-stick orbit remains only in fallback, combat, and unsupported phases. `--native-p2-camera-collision-test` uses the same co-op camera mode but prints the focused centering and world-collision acceptance checklist. A valid run logs `native_second_player_camera_collision=true`, then `event=player_two_native_camera phase=target_verified`, `phase=state_verified`, and finally `phase=ready`. Any `phase=session_disabled` record includes the terminal reason; that record or a missing ready edge is a failed acquisition, not collision acceptance.
...
That general provenance path is isolated and remains back-burnered for the
party-lifecycle restart. The save-book vote does not install or consume its
dispatch, accepted-message, or SOL hooks.
...
`--party-lifecycle-trace` explicitly keeps this old preview and all forge
commits disabled; the checked-in INI also remains false. Sudeki's serialized,
native full-width blacksmith remains the active fallback.
