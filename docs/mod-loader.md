...
`EnableSharedGroupCameraPrototype` defaults to `false` and requires both control separation and second-player movement. It accepts only the standard gameplay case where `CCamera+0xB4/+0xB8` contain the same native `GameObjectTarget`, retains those references, creates Sudeki's native `MatrixTarget`, and replaces only the target translation with the Player 1/Buki midpoint. It restores only slots it still owns when AI, camera, or target state changes and releases every reference through the native manager. This first prototype changes focus only; zoom, distance limits, collision, scope cameras, cinematics, and render views remain unchanged. Build and inert exact-image preflight pass; visual confirmation is pending.
...
`tools/continue-research.sh --shared-group-camera-test` adds the unconfirmed native midpoint target to the camera-relative movement and separation setup. Use `F10` to acquire/release Buki's AI override and verify both midpoint focus and restoration. The launcher does not enable adaptive zoom in this mode.
